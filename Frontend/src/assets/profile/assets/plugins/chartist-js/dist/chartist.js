...
(function (window, document, Chartist) {
  ...
  /**
   * Map for multi dimensional arrays where their nested arrays will be mapped in serial. The output array will have the length of the largest nested array. The callback function is called with letiable arguments where each argument is the nested array value (or undefined if there are no more values).
   *
   * @memberof Chartist.Core
   * @param arr
   * @param cb
  ...
  Chartist.normalizeData = function(data) {
    ...
    if (data.series.length > 0 && data.labels.length === 0) {
      let normalized = Chartist.getDataArray(data),
          labelCount;
  ...
  Chartist.createLabel = function(position, length, index, labels, axis, axisOffset, labelOffset, group, classes, useForeignObject, eventEmitter) {
    ...
    if(useForeignObject) {
      ...
      // 100% in all browsers
      let content = '<span class="' + classes.join(' ') + '" style="' +
...
(function(window, document, Chartist) {
  ...
  Chartist.Interpolation.cardinal = function(options) {
    ...
    return function cardinal(pathCoordinates, valueData) {
      ...
      let segments = splitIntoSegments(pathCoordinates, valueData);
...
(function(window, document, Chartist) {
  ...
  // letiable argument list clones args > 0 into args[0] and retruns modified args[0]
...
(function (window, document, Chartist) {
  ...
  function AutoScaleAxis(axisUnit, data, chartRect, options) {
    ...
    let highLow = options.highLow || Chartist.getHighLow(data.normalized, options, axisUnit.pos);
...
(function(window, document, Chartist){
  ...
  function createChart(options) {
    ...
    if(options.stackBars && data.normalized.length !== 0) {
      ...
      let serialSums = Chartist.serialMap(data.normalized, function serialSums() {
...
(function(window, document, Chartist) {
  ...
  function createChart(options) {
    ...
    let hasSingleValInSeries = this.data.series.filter(function(val) {
    ...
    for (let i = 0; i < this.data.series.length; i++) {
      ...
      let endAngle = startAngle + dataArray[i] / totalDataSum * 360;

      // Use slight offset so there are no transparent hairline issues
      let overlappigStartAngle = Math.max(0, startAngle - (i === 0 || hasSingleValInSeries ? 0 : 0.2));
      ...
      let path = new Chartist.Svg.Path(!options.donut)
        .move(end.x, end.y)
      ...
      let pathElement = seriesGroups[i].elem('path', {
        d: path.stringify()
      ...
      if(options.showLabel) {
        ...
        let labelPosition = Chartist.polarToCartesian(center.x, center.y, labelRadius, startAngle + (endAngle - startAngle) / 2),
