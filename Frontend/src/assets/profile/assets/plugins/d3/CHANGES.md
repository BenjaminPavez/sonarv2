...
To the consternation of some users, 3.x employed Unicode letiable names such as λ, φ, τ and π for a concise representation of mathematical operations. A downside of this approach was that a SyntaxError would occur if you loaded the non-minified D3 using ISO-8859-1 instead of UTF-8. 3.x also used Unicode string literals, such as the SI-prefix µ for 1e-6. 4.0 uses only ASCII letiable names and ASCII string literals (see [rollup-plugin-ascii](https://github.com/mbostock/rollup-plugin-ascii)), avoiding encoding problems.
...
let i = d3.scan(data, function(a, b) { return a.value - b.value; }); // 2
...
```js
let ticks = d3.ticks(0, 10, 5); // [0, 2, 4, 6, 8, 10]
```
...
The method signature for optional accessors has been changed to be more consistent with array methods such as [*array*.forEach](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/forEach): the accessor is passed the current element (*d*), the index (*i*), and the array (*data*), with *this* as undefined. This affects [d3.min](https://github.com/d3/d3-array/blob/master/README.md#min), [d3.max](https://github.com/d3/d3-array/blob/master/README.md#max), [d3.extent](https://github.com/d3/d3-array/blob/master/README.md#extent), [d3.sum](https://github.com/d3/d3-array/blob/master/README.md#sum), [d3.mean](https://github.com/d3/d3-array/blob/master/README.md#mean), [d3.median](https://github.com/d3/d3-array/blob/master/README.md#median), [d3.quantile](https://github.com/d3/d3-array/blob/master/README.md#quantile), [d3.letiance](https://github.com/d3/d3-array/blob/master/README.md#letiance) and [d3.deviation](https://github.com/d3/d3-array/blob/master/README.md#deviation). The [d3.quantile](https://github.com/d3/d3-array/blob/master/README.md#quantile) method previously did not take an accessor. Some methods with optional arguments now treat those arguments as missing if they are null or undefined, rather than strictly checking arguments.length.
...
```js
let yields = [
  {yield: 22.13333, letiety: "Manchuria",        year: 1932, site: "Grand Rapids"},
  {yield: 26.76667, letiety: "Peatland",         year: 1932, site: "Grand Rapids"},
  {yield: 28.10000, letiety: "No. 462",          year: 1931, site: "Duluth"},
  {yield: 38.50000, letiety: "Svansota",         year: 1932, site: "Waseca"},
  {yield: 40.46667, letiety: "Svansota",         year: 1931, site: "Crookston"},
  {yield: 36.03333, letiety: "Peatland",         year: 1932, site: "Waseca"},
  {yield: 34.46667, letiety: "Wisconsin No. 38", year: 1931, site: "Grand Rapids"}
];

let sites = d3.set(yields, function(d) { return d.site; }); // Grand Rapids, Duluth, Waseca, Crookston
```
...
```js
let red = d3.color("hsl(0, 80%, 50%)"); // {h: 0, l: 0.5, s: 0.8, opacity: 1}
```
...
Pursuant to the great namespace flattening, letious CSV and TSV methods have new names:
...
```js
let string = d3.csvFormat(data, ["Year", "Model", "Length"]);
```
...
¹ The -in, -out and -in-out letiants of linear easing are identical, so there’s just d3.easeLinear.
...
```js
let e = d3.ease("elastic-out-in", 1.2);
```

The equivalent in D3 4.0 is:

```js
let e = d3.easeElastic.amplitude(1.2);
```
...
Pursuant to the great namespace flattening, letious methods have new names:
...
```js
let fx = typeof x === "function" ? x : constant(x);
```
...
```js
let interpolate = d3.interpolateRgb.gamma(2.2)("purple", "orange");
```
...
```js
let parseTime = d3.time.format("%c").parse;
```
