...
let ps = require('../main');
let psInstances = require('../plugin/instances');
...
let EventElement = function (element) {
...
EventElement.prototype.unbind = function (eventName, handler) {
  let isHandlerProvided = (typeof handler !== 'undefined');
...
let cls = require('./class');
let dom = require('./dom');

let toInt = exports.toInt = function (x) {
  return parseInt(x, 10) || 0;
};

let clone = exports.clone = function (obj) {
  if (!obj) {
    return null;
...
exports.removePsClasses = function (element) {
  let clsList = cls.list(element);
  for (let i = 0; i < clsList.length; i++) {
    let className = clsList[i];
...
let destroy = require('./plugin/destroy');
let initialize = require('./plugin/initialize');
let update = require('./plugin/update');
...
let instances = require('../instances');
let updateGeometry = require('../update-geometry');
let updateScroll = require('../update-scroll');
...
function bindClickRailHandler(element, i) {
  ...
  i.event.bind(i.scrollbarYRail, 'click', function (e) {
    let positionTop = e.pageY - window.pageYOffset - pageOffset(i.scrollbarYRail).top;
    let direction = positionTop > i.scrollbarYTop ? 1 : -1;
  ...
  i.event.bind(i.scrollbarXRail, 'click', function (e) {
    let positionLeft = e.pageX - window.pageXOffset - pageOffset(i.scrollbarXRail).left;
...
function bindMouseScrollXHandler(element, i) {
  ...
  function updateScrollLeft(deltaX) {
    let newLeft = currentLeft + (deltaX * i.railXRatio);
    let maxLeft = Math.max(0, i.scrollbarXRail.getBoundingClientRect().left) + (i.railXRatio * (i.railXWidth - i.scrollbarXWidth));
    ...
    let scrollLeft = _.toInt(i.scrollbarXLeft * (i.contentWidth - i.containerWidth) / (i.containerWidth - (i.railXRatio * i.scrollbarXWidth))) - i.negativeScrollAdjustment;
...
function bindMouseScrollYHandler(element, i) {
  ...
  function updateScrollTop(deltaY) {
    let newTop = currentTop + (deltaY * i.railYRatio);
    let maxTop = Math.max(0, i.scrollbarYRail.getBoundingClientRect().top) + (i.railYRatio * (i.railYHeight - i.scrollbarYHeight));
    ...
    let scrollTop = _.toInt(i.scrollbarYTop * (i.contentHeight - i.containerHeight) / (i.containerHeight - (i.railYRatio * i.scrollbarYHeight)));
...
function bindMouseWheelHandler(element, i) {
  ...
  function shouldBeConsumedByChild(deltaX, deltaY) {
    ...
    if (child) {
      ...
      let maxScrollTop = child.scrollHeight - child.clientHeight;
      if (maxScrollTop > 0) {
        if (!(child.scrollTop === 0 && deltaY > 0) && !(child.scrollTop === maxScrollTop && deltaY < 0)) {
          return true;
        }
      }
      let maxScrollLeft = child.scrollLeft - child.clientWidth;
      if (maxScrollLeft > 0) {
...
function bindTouchHandler(element, i, supportsTouch, supportsIePointer) {
  function shouldPreventDefault(deltaX, deltaY) {
    let scrollTop = element.scrollTop;
    let scrollLeft = element.scrollLeft;
    let magnitudeX = Math.abs(deltaX);
    let magnitudeY = Math.abs(deltaY);
...
let nativeScrollHandler = require('./handler/native-scroll');
...
let _ = require('../lib/helper');
let cls = require('../lib/class');
let defaultSettings = require('./default-setting');
let dom = require('../lib/dom');
let EventManager = require('../lib/event-manager');
...
function updateCss(element, i) {
  ...
  let yRailOffset = {top: element.scrollTop, height: i.railYHeight};
...
let createDOMEvent = function (name) {
